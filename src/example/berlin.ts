...
export const BerlinGroupIsoDefinition = createFieldDefinition({
  fields: {
    ...
    '10': { name: 'Conversion Rate, Cardholder Billing', field: N({ length: 8 }) },
    '11': { name: 'System Trace Audit Number (STAN)', field: N({ length: 6 }) },
    '12': { name: 'Date and Time, Local Transaction', field: AN({ length: 12 }) },
    ...
    '32': { name: 'Acquiring Institution Identification Code', field: LLVAR_N },
    ...
    '41': { name: 'Card Acceptor Terminal Identification', field: AN({ length: 8 }) },
    '42': { name: 'Card Acceptor Identification Code', field: AN({ length: 15 }) },
    ...
    '51': { name: 'Currency Code, Cardholder Billing', field: N({ length: 3 }) },
    '52': { name: 'Personal Identification Number (PIN) Data', field: B({ length: 8 }) },
    '53': { name: 'Security Related Control Information', field: LLVAR_B },
    '54': { name: 'Amounts, Additional', field: LLLVAR_AN },
    '55': { name: 'Integrated Circuit Card (ICC) System Related Data', field: LLLVAR_B },
    '56': { name: 'Original Data Elements', field: LLVAR_N },
    '57': { name: 'Authorization Life Cycle Code', field: N({ length: 3 }) },
    '58': { name: 'Authorizing Agent Institution Identification Code', field: LLVAR_N },
    '59': { name: 'Acquirer Reference Data (Transport Data) ', field: LLLVAR_AN },
    '62': { name: 'e-Payment Data', field: LLLVAR_AN },
    '64': { name: 'Message Authentication Code (MAC) Field', field: B({ length: 8 }) },
    '95': { name: 'Card Issuer Reference Data', field: LLVAR_AN },
    '111': { name: 'Encryption Data', field: LLLLVAR_B },
    '128': { name: 'Message Authentication Code (MAC) Field', field: B({ length: 8 }) },
  },
  mtiField: N({ length: 4 }),
});
